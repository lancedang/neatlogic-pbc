...
@Transactional
public class SaveInterfaceApi extends PrivateApiComponentBase {
    ...
    @Input({@Param(name = "id", type = ApiParamType.STRING, isRequired = true, maxLength = 100, desc = "接口id"),
            @Param(name = "name", type = ApiParamType.STRING, isRequired = true, maxLength = 100, desc = "接口名称")})
    @Description(desc = "保存接口定义接口")
    @Override
    public Object myDoService(JSONObject paramObj) throws Exception {
        InterfaceVo interfaceVo = JSONObject.toJavaObject(paramObj, InterfaceVo.class);
        InterfaceVo oldInterfaceVo = interfaceMapper.getInterfaceById(interfaceVo.getId());
        if (oldInterfaceVo == null) {
            interfaceMapper.insertInterface(interfaceVo);
        } else {
            interfaceMapper.updateInterface(interfaceVo);
        }
        return null;
    }
