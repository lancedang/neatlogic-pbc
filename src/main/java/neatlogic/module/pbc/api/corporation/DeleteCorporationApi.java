...
import neatlogic.framework.pbc.exception.CorporationHasItemException;
...
public class DeleteCorporationApi extends PrivateApiComponentBase {
    @Resource
    ...
    @Description(desc = "删除机构配置信息接口")
    @Input({@Param(name = "id", type = ApiParamType.LONG, isRequired = true, desc = "id")})
    @Override
    public Object myDoService(JSONObject paramObj) throws Exception {
        Long id = paramObj.getLong("id");
        int count = interfaceItemMapper.getItemCountByCorporationId(id, null);
        if (count > 0) {
            throw new CorporationHasItemException(count);
        }
        corporationMapper.deleteCorporation(id);
        return null;
    }

    @Override
    public String getToken() {
        return "/pbc/corporation/delete";
    }
}
